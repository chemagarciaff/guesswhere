...
const PantallaRanking = () => {
  ...
  const { avatares, usuario } = useContext(MapaContext)
  ...
  return (
    <div className="w-full min-h-screen relative top-0 left-0 fondo-mapa flex flex-col items-center justify-start pt-[110px] gap-8 ">
      ...
      <div className='px-12 md:px-[70px] lg:px-[100px] xl:px-[150px] gap-10 flex flex-col overflow-hidden w-full pb-4 fadeUp'>

        <div className="flex-1 overflow-hidden bg-gray-100 bg-opacity-30 backdrop-blur-sm p-6 rounded-lg shadow-lg fadeUp borde-arcoiris ">
          <div className=" overflow-y-auto">

           <table className=" w-full text-sm sm:text-base text-center table-fixed scroll-invisible">
  <thead className="border text-white/80 border-b border-white/30 bg-white/10">
    <tr>
      <th className="py-3 w-[10%]">Puesto</th>
      <th className="py-3 w-[10%] hidden lg:flex">Avatar</th>
  ...
  <tbody className="text-white divide-y divide-white/15">
    {usuariosPublicos.map((usuari, index) => {
      ...
      return (
        <tr key={usuari.id_jugador} className={`hover:bg-white/10 transition ${usuari.id_jugador == usuario.id_jugador ? 'bg-slate-500' : ''}`}>
          <td className={`${baseRow} ${rankStyle}`}>{index + 1}</td>
          <td className={`${baseRow}  hidden lg:flex`}>
            <div className="flex justify-center">
              <img
                src={avatares[usuari.id_jugador]}
                alt={usuari.username}
                className="w-6 h-6 rounded-full object-cover border border-white/40"
              />
            </div>
          </td>
          <td className={baseRow}>{usuari.username}</td>
          <td className={baseRow}>{usuari.puntuacion_total}</td>
          <td className={baseRow}>{usuari.fecha_registro.split("T")[0]}</td>
        </tr>
      );
    })}
  </tbody>
